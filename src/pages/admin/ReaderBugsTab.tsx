...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="font-mono text-[28px] leading-none text-red">{value}</div>
      <div className="mt-2.5 font-mono text-[9.8px] font-medium tracking-[0.16em] text-mute">{label}</div>
    </div>
  )
}

export function ReaderBugsTab() {
  ...
  if (reports === undefined) {
    return (
      <div>
        <div className="mb-6.5 flex flex-wrap gap-6 border-b border-ink/14 pb-6.5 md:gap-10">
          {[0, 1].map((i) => (
            <div key={i}>
              <Skel className="mb-2.5 h-[28px] w-14" />
